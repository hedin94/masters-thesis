...
Since \(\Delta(\mathbf{v})\) is quadratic, finding a \(\mathbf{v}\) which gives a minimal error is a linear problem. The best position \(\mathbf{\bar{v}}_i\) for \(v_i\) after a collapse \((v_j, v_i) \rightarrow v_i\) is a solution to equation~\ref{eq:quadric_opt}.
...
\subsection{Considering appearance attributes}
...
\subsection{Considering appearance attributes} \label{sec:texture_mapped_progressive_meshing}
